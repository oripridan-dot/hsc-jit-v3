...
import { unifiedStateManager, componentBridge } from './src/store/unifiedRouter';
...
function useUnifiedQuery(source: 'explorer' | 'promptbar') {
  ...
  useEffect(() => {
    ...
    const handleContext = (context: any) => {
    ...
    unifiedStateManager.subscribe('context', handleContext);
    ...
    return () => {
      ...
      unifiedStateManager.unsubscribe('context', handleContext);
  ...
  const submitQuery = useCallback((queryText: string, filters?: any) => {
    ...
    if (source === 'explorer') {
      componentBridge.handleExplorerQuery(queryText, filters);
    } else {
      componentBridge.handlePromptBarQuery(queryText);
    }
  }, [source]);

  const selectProduct = useCallback((productId: string) => {
    componentBridge.handleProductSelection(productId, source);
  }, [source]);
        ...
        <span>Detected Products: </span>
          ...
          < div className = "suggestions-list" >
          {
            predictions.slice(0, 5).map((product) => (
              <button
                key= { product.id }
                className = {`suggestion-chip ${selectedProduct?.id === product.id ? 'selected' : ''
                }`}
