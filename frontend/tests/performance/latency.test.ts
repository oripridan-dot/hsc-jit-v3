...
describe('Performance Tests', () => {
    ...
    describe('Search Latency', () => {
        ...
        it('should search <100ms for 10 sequential queries', () => {
            const queries = ['drums', 'synth', 'juno', 'roland', 'td', 'fp', 'jupiter', 'kit', 'patch', 'keys'];
            const startTime = performance.now();
            ...
            console.log(`10 searches: ${duration.toFixed(2)}ms (avg: ${(duration / queries.length).toFixed(2)}ms)`);
        });
    ...
    describe('Memory Efficiency', () => {
        it('should handle product set without memory spikes', () => {
            ...
            for (let i = 0; i < iterations; i++) {
                const filtered = allProducts.filter(p => p.category === 'Electronic Drums');
            ...
            console.log(`Memory increase over ${iterations} iterations: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`);
        });
    });
