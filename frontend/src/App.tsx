...
function App() {
  ...
  const [fullProducts, setFullProducts] = useState<unknown[]>([]);
  ...
  return (
    <div className="flex fixed inset-0 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-slate-50 overflow-hidden font-sans selection:bg-cyan-500/30">
      <div className="flex w-full h-full">
        <div className="w-80 h-full flex-shrink-0 border-r border-slate-800/50 bg-slate-950/80 backdrop-blur-sm">
          <Navigator />
        </div>
        <div className="flex-1 h-full flex flex-col">
          <div className="h-14 border-b border-slate-800/50 flex items-center px-6 gap-4 bg-slate-950/80 backdrop-blur-md z-20 shadow-lg">
            ...
            <button onClick={() => setAiAssistantOpen(!aiAssistantOpen)} className={`px-4 py-2 rounded-lg text-xs font-bold tracking-wider transition-all shadow-md ${aiAssistantOpen ? 'bg-cyan-500/30 text-cyan-300 border-2 border-cyan-400/60 shadow-cyan-500/30' : 'bg-slate-800/60 text-slate-300 border-2 border-slate-700/60 hover:border-cyan-500/50 hover:text-cyan-400'}`}>🤖 HALILEO</button>
          </div>
          <div className="flex-1 overflow-hidden">
            <Workbench />
          </div>
        </div>
        <div className={`h-full border-l border-slate-800/50 bg-slate-950/50 backdrop-blur-md transition-all duration-300 ease-in-out ${aiAssistantOpen ? 'w-96' : 'w-0'} overflow-hidden`}>
          {aiAssistantOpen && (
            <div className="h-full flex flex-col">
              <div className="h-14 border-b border-slate-800/50 flex items-center px-6 gap-3">
                <div className="text-xs font-mono font-bold text-emerald-400 tracking-wider">ANALYST</div>
                <div className="flex-1"></div>
