...
// SearchGraphItem interface removed (unused)
...
const getCategoryIcon = (category: string) => {
  ...
  if (lower.includes("key") || lower.includes("piano"))
    return <Piano size={14} className="text-indigo-400" />;
  if (lower.includes("drum") || lower.includes("perc"))
    return <Music size={14} className="text-purple-400" />;
  if (lower.includes("speaker") || lower.includes("audio"))
    return <Speaker size={14} className="text-blue-400" />;
  if (lower.includes("mic")) return <Mic2 size={14} className="text-red-400" />;
  if (lower.includes("cable") || lower.includes("stand"))
...
/**
 * Helper to safely find images by type from unknown input
 */
const findImageByType = (
  ...
  return (images as any[]).find((img): img is ProductImage => {
...
export const Navigator: React.FC<NavigatorProps> = ({
  ...
  // Removed unused brandIdentities state
  const {
    whiteBgImages,
    selectBrand,
    selectCategory,
  ...
  useEffect(() => {
    const loadCatalog = async () => {
      try {
        ...
        // @ts-expect-error - Schema and Interface slightly differ in strict mode but structure is compatible
        ...
        console.log(
          `✅ Halilit Catalog loaded: ${data.brands.length} brands, ${data.total_products} products`,
        );
      } catch (err) {
  ...
  const loadBrandProducts = async (slug: string) => {
    ...
    try {
      ...
      const brandEntry = catalogIndex?.brands.find(
      ...
      const filePath = fileName.startsWith("/data/")
        ? fileName
        : `/data/${fileName}`;
      ...
      if (data.products && Array.isArray(data.products)) {
        data.products.forEach((product: Product) => {
          if (
            ...
            // White background detection handled by image processing
            const galleryImage = findImageByType(product.images, "gallery");
            const mainImage = findImageByType(product.images, "main");

            const galleryUrl = galleryImage ? galleryImage.url : undefined;
            const mainUrl = mainImage ? mainImage.url : undefined;
      ...
      console.log(
        `✅ Loaded ${slug}: ${data.products?.length || 0} products with hierarchy`,
      );
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : "Unknown error";
   ...
   */
  const buildHierarchyFromProducts = (
    products: Product[],
                    ...
                    return (
                      <div key={brandId} className="space-y-1">
                        ...
                        >
                          ...
                          <ChevronRight
                            className={`w-4 h-4 text-[var(--text-tertiary)] transition-transform flex-shrink-0 ${isExpanded ? "rotate-90" : ""}`}
                          />
                        </button>
                          ...
                          >
                            {loadingBrands.has(brandId) ? (
                              ...
                              <div className="space-y-1">
                                {Object.entries(products.hierarchy).map(
                                      ...
                                      >
                                        ...
                                        >
                                          <div className="flex items-center gap-2 flex-1 min-w-0">
                                            <ChevronRight
                                              className={`w-3 h-3 text-indigo-400 flex-shrink-0 transition-transform ${isCategoryExpanded ? "rotate-90" : ""}`}
                                            />
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        <div
                          className={`text-sm font-bold truncate ${currentUniversalCategory === cat.id ? "text-indigo-400" : "text-[var(--text-primary)]"}`}
                        >
                          {cat.label}
                        </div>
                    ...
                    <ChevronRight
                      className={`w-4 h-4 text-[var(--text-tertiary)] flex-shrink-0 ${currentUniversalCategory === cat.id ? "text-indigo-400" : ""}`}
                    />
                  </button>
                ))}
              </motion.div>
            ...
            >
              {isSearching ? (
                ...
                <div className="flex flex-col items-center justify-center h-40 space-y-3">
                  ...
                  <p className="text-sm text-[var(--text-secondary)]">
                    Enter a search below to get started
                  </p>
                </div>
              )}
            </motion.div>
