...
class MoogProcessor:
    def normalize(self, raw_payload: Dict[str, Any]) -> Dict[str, Any]:
        """
        Transforms Raw Moog Data -> Standardized Product Blueprint
        """
        model_name = raw_payload.get('model', 'Unknown')
        name = raw_payload.get('name', model_name)
        brand = raw_payload.get('brand', 'Moog')
        ...
        if isinstance(raw_specs, list):
            for item in raw_specs:
                ...
                # TODO: Add Moog-specific spec categorization logic
        ...
        blueprint = {
            "id": f"moog-{model_name.lower().replace(' ', '_')}",
            "brand": brand,
            "name": name,
            "sku": raw_payload.get('sku'),
            "main_category": raw_payload.get('hierarchy', {}).get('category', 'Synths'), # Inference fallback
